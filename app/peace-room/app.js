...
/* ── Dev Tools — Run AI Analysis ────────────────────── */
// Show dev controls only in local dev (data.source === "ai-analyzer-prod" && localhost)
function initDevControls() {
  const devDiv = document.getElementById('devControls');
  if (!devDiv) return;
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch('/api/analysis/run?mode=--fast');
  ...
  async function pollAnalysisStatus() {
    const poll = async () => {
      try {
        ...
        status.textContent = 'Running… (PID ' + s.pid + ')';
...
// Version tag is now rendered in renderAll() for access to data.aiVersion
window.addEventListener('DOMContentLoaded', initDevControls);
